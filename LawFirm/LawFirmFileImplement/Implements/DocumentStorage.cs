...
namespace LawFirmFileImplement.Implements
{
    public class DocumentStorage : IDocumentStorage
    {
        ...
        {
            return source.Documents.Select(CreateModel).ToList();
        }
        public List<DocumentViewModel> GetFilteredList(DocumentBindingModel model)
        {
            ...
            return source.Documents.Where(rec => rec.DocumentName.Contains(model.DocumentName)).Select(CreateModel).ToList();
        }
        public DocumentViewModel GetElement(DocumentBindingModel model)
        {
            ...
            var document = source.Documents.FirstOrDefault(rec => rec.DocumentName == model.DocumentName || rec.Id == model.Id);

            return document != null ? CreateModel(document) : null;
        }
        public void Insert(DocumentBindingModel model)
        {
            int maxId = source.Documents.Count > 0 ? source.Components.Max(rec => rec.Id) : 0;

            var element = new Document { Id = maxId + 1, DocumentComponents = new Dictionary<int, int>() };
        ...
        private DocumentViewModel CreateModel(Document document)
        {
            return new DocumentViewModel
            {
                ...
                DocumentComponents = document.DocumentComponents.ToDictionary(recPC => recPC.Key, recPC =>
